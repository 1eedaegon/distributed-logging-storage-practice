...
type Log struct {
	mu     sync.RWMutex // Write mutex가 아니면 언제든지 읽을 수 있다.
	Dir    string
	Config Config
...
func (l *Log) Append(record *api.Record) (uint64, error) {
	// 추가 연산은 원자적 연산을 해야한다
	// Log는 RWMutex이므로 쓰기 연산이 아니면 읽을 수 있다.
	l.mu.Lock()
	defer l.mu.Unlock()

	if l.activeSegment.IsMaxed() { // 가득차면 새로운 activeSegment를 만들고 할당
...
// Reader interface must be implemented read()
...
// 전체 원본 파일을 읽기 위한 multi reader 구현
